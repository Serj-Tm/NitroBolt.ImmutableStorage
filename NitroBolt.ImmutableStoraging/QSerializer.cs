...
namespace NitroBolt.ImmutableStoraging
{
  ...
  {
    public QSerializer(Dictionary<Type, string> ids, Dictionary<Type, Dictionary<string, string>> references, Dictionary<Type, Dictionary<string, PushInfo[]>> pushes)
    {
    ...
    static readonly Dictionary<Type, ImmutablePropertyInfo[]> PropertyIndex = new Dictionary<Type, ImmutablePropertyInfo[]>();
    ...
    {
      public SerializeTypeInfo(ImmutablePropertyInfo idProperty, ImmutablePropertyInfo[] properties, Dictionary<string, string> refProperties,
            ...
            .Select(parameter =>
              {
                ...
                return new SerializeParameterInfo(pName, parameter.ParameterType, IdProperty?.Name == pName, refProperties.Find(pName), pushes.Find(pName));
              }
            )
            .ToArray();
          this.ParameterIndexByName = this.Parameters.Select((p, i) => new { p.Name, i }).ToDictionary(pair => pair.Name, pair => pair.i);
    ...
    {
      public SerializeParameterInfo(string name, Type parameterType, bool isId, string refProperty, PushInfo[] push)
      {
        this.Name = name;
      ...
      {
        return string.Format("{0}{1} {2}", IsId ? "*" : "", Name, ParameterType.Name);
      }
    }
    ...
    public class Loader
    {
      public Loader(Type type, Func<Type, bool> isTypeLoading, Func<Type, QSharp.QNode, Dictionary<string, Dictionary<object, object>>, object> load)
      {
        this.Type = type;
    ...
    public static readonly Loader[] Loaders = new[]
      {
        ...
        new Loader(typeof(int), null, (type, q, data) => ToInt(q.Value?.ToString())),
        new Loader(typeof(long), null, (type, q, data) => ToLong(q.Value?.ToString())),
        ...
        new Loader(typeof(DateTime), null, (type, q, data) => DateTime.Parse(q.Value?.ToString(), RuCulture)),

        new Loader(null, type => type.IsEnum, (type, q, data)=>Enum.Parse(type, q.Value?.ToString())),
      };

    public readonly Dictionary<Type, Func<Type, QSharp.QNode, Dictionary<string, Dictionary<object, object>>, object>> LoaderIndex = 
    ...
    public object LoadStructure(Type type, QSharp.QNode q, Dictionary<string, Dictionary<object, object>> data)
    {
      ...
      for (var i = 0; i < typeInfo.Parameters.Length; ++i)
      {
        ...
        foreach (var push in prop_pushes)
        {
          ...
          data[push.Name] = Else_Empty(val.As<System.Collections.IEnumerable>()).OfType<object>().ToDictionary(obj => GetId(obj, elementTypeInfo.IdProperty));
        }
      }
    ...
    private object Load_Ref(string targetRef, Type type, QSharp.QNode[] qs, Dictionary<string, Dictionary<object, object>> data)
    {
    ...
    object Load_s(Type type, QSharp.QNode[] qs, Dictionary<string, Dictionary<object, object>> data)
    {
      if (IsCollectionType(type))
      {
        ...
        if (type.IsGenericType)
        {
          ...
          return create.MakeGenericMethod(element_type).Invoke(null, new[] { array });
        }
        return array;
      }
    ...
    QSharp.QNode[] SaveProp(object item, ImmutablePropertyInfo prop, bool isRef = false)
    {
      ...
      if (IsCollectionType(prop.Type))
      {
        ...
        return child != null ? new[] { Save(child, isRef, itemType: prop.ReducedType != typeof(object) ? prop.ReducedType : null) } : Array<QSharp.QNode>.Empty;
    }
    ...
    public static IEnumerable<ImmutablePropertyInfo> Childs(Type type, bool isOnlyProperties = false)
    {
      ...
      var childs = type.GetFields().Where(field => !field.IsStatic).Select(field => new ImmutablePropertyInfo(field, null, null, field.FieldType, null))
         .Concat(type.GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public).Select(property => new ImmutablePropertyInfo(null, property, null, property.PropertyType, null)));
      if (!isOnlyProperties)
        childs = childs.Concat(type.GetConstructors().SelectMany(constructor => constructor.GetParameters().Select(parameter => new ImmutablePropertyInfo(null, null, parameter.Name, parameter.ParameterType, null))));

      foreach (var child in childs)
      {
        yield return new ImmutablePropertyInfo(child.Field, child.Property, child.InitialName, child.Type, ReduceType(child.Type));
      }
    }
    ...
    public static bool IsCollectionType(Type type)
    {
      ...
      if (type.IsArray)
        return true;
      return type.IsGenericType && type.GetGenericTypeDefinition() == typeof(System.Collections.Immutable.ImmutableArray<>);
    }
...
",
  ...
  {
    public ImmutablePropertyInfo(System.Reflection.FieldInfo field, System.Reflection.PropertyInfo property, string initialName, Type type, Type reducedType)
    {
      this.Field = field;
      this.Property = property;
    ...
    public string Name { get { return Field != null ? Field.Name : Property != null ? Property.Name : null; } }
    ...
    public override string ToString()
    {
      return string.Format("{0}:{1}{2}", Name ?? InitialName, Type != null ? Type.Name : null, Type != ReducedType ? string.Format("({0})", ReducedType.Name) : null);
    }
  }
}